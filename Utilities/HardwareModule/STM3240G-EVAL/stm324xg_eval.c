...
const uint16_t COM_SPI_CS0_PIN[SPIn] = {
    ...
    /*structure for SPI CS1 Pins*/
GPIO_TypeDef* COM_SPI_CS1_PORT[SPIn] = {
    USER_SPI1_CS1_PORT,
    USER_SPI2_CS1_PORT};
const uint32_t COM_SPI_CS1_CLK[SPIn] = {
    USER_SPI1_CS1_CLK,
    USER_SPI2_CS1_CLK};
const uint16_t COM_SPI_CS1_PIN[SPIn] = {
    USER_SPI1_CS1_PIN,
    USER_SPI2_CS1_PIN};
...
/**
  ...
  * @retval None
  */
void UserButtonInit(Button_TypeDef Button, ButtonMode_TypeDef Button_Mode)
{
  GPIO_InitTypeDef GPIO_InitStructure;
...
/**
  ...
  */
uint32_t UserButtonGetState(Button_TypeDef Button)
{
...
void UserSpiInit(USER_SPI_TypeDef COM)
{
  ...
  RCC_AHB1PeriphClockCmd(COM_SPI_SCK_CLK[COM] 
                         | COM_SPI_MISO_CLK[COM] 
                             | COM_SPI_MOSI_CLK[COM] 
                                 | COM_SPI_CS0_CLK[COM] 
                                     | COM_SPI_CS1_CLK[COM]|RCC_AHB1Periph_GPIOI , ENABLE);
  ...
  GPIO_Init(GPIOI, &GPIO_InitStructure);
    /*Config Output type for MISO Pins*/
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
  GPIO_Init(GPIOI, &GPIO_InitStructure);
    /*Config Output type for CS0 Pins*/
  GPIO_InitStructure.GPIO_Pin = COM_SPI_CS0_PIN[COM];
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_Init(COM_SPI_CS0_PORT[COM], &GPIO_InitStructure);
  /*Config Output type for CS1 Pins*/
  GPIO_InitStructure.GPIO_Pin = COM_SPI_CS1_PIN[COM];
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_Init(COM_SPI_CS1_PORT[COM], &GPIO_InitStructure);
    
  /*Config alternate function for SPI Module*/
  GPIO_PinAFConfig(GPIOI,GPIO_PinSource1,COM_SPI_AF[COM]);/*SCK Pins*/
  GPIO_PinAFConfig(GPIOI,GPIO_PinSource2,COM_SPI_AF[COM]);/*MISO Pins*/
  GPIO_PinAFConfig(GPIOI,GPIO_PinSource3,COM_SPI_AF[COM]);/*MOSI Pins*/
  ...
  //  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4; clock 72 MHZ
  //SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_128;//clock 84MHZ (168Mhz)
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_256;//clock 84MHZ (168Mhz)
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
  SPI_InitStructure.SPI_CRCPolynomial = 7;
  SPI_Init(COM_SPI[COM], &SPI_InitStructure);
  
  /*Enable SPI1*/
  SPI_Cmd(COM_SPI[COM], ENABLE);
  
  /*Set chip select to deactive level*/
  COM_SPI_CS0_PORT[COM]->BSRRL = COM_SPI_CS0_PIN[COM];
  COM_SPI_CS1_PORT[COM]->BSRRL = COM_SPI_CS1_PIN[COM];
}  
...
void UpdateHistoryLog(structNodeAddressing* p_strtNodeAddr,
                      enumDeviceAddr eDevideAddr,
                      uint8 bValue,
                      RTC_TimeTypeDef* p_strtTimeInfo,
                      RTC_DateTypeDef* p_strtDateInfo)
{
    /*Change values to string*/
    char bTempChar[3];
    memset(bTempChar,0,sizeof(bTempChar));
    if (strlen(BODY_PAGE_HISTORY_LOG)<HISTORY_LOG_SIZE)
    {
        strcat((char*)BODY_PAGE_HISTORY_LOG,"<br>F");
        if (p_strtNodeAddr->bFloorAddr==eFLOOR_B)
            bTempChar[0] = 'B';
        else if (p_strtNodeAddr->bFloorAddr==eFLOOR_G)
            bTempChar[0] = 'G';
        else
            bTempChar[0] = p_strtNodeAddr->bFloorAddr +'0'-1;
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG," R");
        bTempChar[0] = p_strtNodeAddr->bRoomAddr +'0'+1;
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG," N");
        bTempChar[0] = p_strtNodeAddr->bNodeBoardAddr +'0'+1;
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG," D");
        bTempChar[0] = eDevideAddr +'0'+1;
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        if (bValue)
            strcat((char*)BODY_PAGE_HISTORY_LOG,"     ON    ");
        else
            strcat((char*)BODY_PAGE_HISTORY_LOG,"     OFF   ");
        
        bTempChar[0] = '0' + ((p_strtTimeInfo->RTC_Hours&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtTimeInfo->RTC_Hours&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG,":");
        bTempChar[0] = '0' + ((p_strtTimeInfo->RTC_Minutes&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtTimeInfo->RTC_Minutes&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG,":");
        bTempChar[0] = '0' + ((p_strtTimeInfo->RTC_Seconds&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtTimeInfo->RTC_Seconds&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        if (p_strtTimeInfo->RTC_H12)
            strcat((char*)BODY_PAGE_HISTORY_LOG,"AM");
        else
            strcat((char*)BODY_PAGE_HISTORY_LOG,"PM"); 
        strcat((char*)BODY_PAGE_HISTORY_LOG,"  ");
        bTempChar[0] = '0' + ((p_strtDateInfo->RTC_Date&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtDateInfo->RTC_Date&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG,"/");
        bTempChar[0] = '0' + ((p_strtDateInfo->RTC_Month&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtDateInfo->RTC_Month&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
        strcat((char*)BODY_PAGE_HISTORY_LOG,"/20");
        bTempChar[0] = '0' + ((p_strtDateInfo->RTC_Year&0xF0)>>4);
        bTempChar[1] = '0' + (p_strtDateInfo->RTC_Year&0x0F);
        strcat((char*)BODY_PAGE_HISTORY_LOG,bTempChar);
    }
}
...
* Return         : None
*******************************************************************************/
void T1Us_DelayUs(unsigned long lValue)
{
  TIM_SetCounter(TIMER_BASE_1US,0);
  while (TIM_GetCounter(TIMER_BASE_1US)<lValue);
}
...
* Return         : None
*******************************************************************************/
void T1Us_DelayMs(unsigned long lValue)
{
  uint8 i=10;
  lValue = lValue/10;
  while (lValue--)
...
void TimerDelayBaseUs_Init(void)
{
  ...
  TIM_TimeBaseStructure.TIM_Period = 0xFFFF;//1us ngat 1 lan
